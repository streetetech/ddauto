...
router.get("/search/:brand/:model", async (req, res) => {
  try {
    const brand = req.params.brand; // Get the brand from the URL params
    const model = req.params.model; // Get the model from the URL params

    const files = await gfs
      .find({ "metadata.brand": brand, "metadata.model": model })
      .toArray(); // Find files matching the brand and model
    if (!files || files.length === 0) {
      return res.status(404).send("No files found");
    }
